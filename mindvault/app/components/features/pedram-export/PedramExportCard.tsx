...
const PedramExportCard: React.FC<PedramExportProps> = ({ files, isCollapsed, onClose }) => {
  const [showModal, setShowModal] = useState(false);
  
  // Function to handle button click to show the modal
  ...
  const handleCloseModal = () => {
    setShowModal(false);
    // If there's an external onClose handler, call it
    if (onClose) {
      onClose();
    }
  };
  ...
  return (
    <>
      {/* Pedram Export Button */}
      <button
        onClick={handleShowModal}
        className={`text-sm px-3 py-1 rounded-lg transition-all duration-200 font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 flex items-center gap-2 border border-blue-200 ${
          isCollapsed ? 'w-full justify-center mt-4' : ''
        }`}
      >
        <FileText size={16} />
        {!isCollapsed && <span>Pedram Analysis</span>}
      </button>
      
      {/* Modal Content */}
      {showModal && <PedramExport files={files} onClose={handleCloseModal} />}
    </>
  );
};
...
Format the memo in HTML with the following styling guidelines:
- Wrap the entire memo in a <div class="investment-memo"> tag
- Use proper heading tags (h1, h2, h3, h4) for section titles
- Create a header section with company name, description, and basic info in a grid layout
- Use tables for structured data with appropriate headers and formatting
- Use highlight spans for important metrics and figures
- Create special box sections for metrics, risks, and opportunities using the metric-box, risk-box, and opportunity-box classes
- Include page breaks where appropriate using <div class="page-break"></div>
- End the document with a footer section

Here is the data to analyze:
${qaJson}

Based on this information, generate a comprehensive investment memo that would help
an investor make an informed decision about this opportunity. Be sure to extract and highlight 
key metrics like valuation, funding amount, market size, and projected returns.
      `;
